...
export const UOMModal: React.FC<UOMModalProps> = ({ isOpen, uom, onClose }) => {
  const { language, saveUOM } = useApp();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const finalUom: UOM = {
      id: uom?.id || formData.id || `uom-${Date.now()}`,
      ...
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-100 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-xs">
          ...
          <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
            ...
            <button
              type="submit"
              className="px-4 py-2 text-xs font-bold text-white bg-orange-600 hover:bg-orange-700 rounded-lg shadow-sm transition flex items-center gap-1.5"
            >
              <Save className="w-3.5 h-3.5" />
              <span>{isAr ? 'حفظ الوحدة' : 'Save UOM'}</span>
            </button>
          </div>
        </form>
      </div>
